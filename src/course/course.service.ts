...
export class CourseService {
    ...
    async createCourse(createDto: CreateCourseDto) {
        ...
        try {
            ...
            for (const subCat of subCategories) {
                if (!categoryIdSet?.has(subCat?.categoryIds?.toString())) {
                    await session.abortTransaction();
                    session.endSession();
            ...
            return { statusCode: 200, message: 'Course created', data: course?.[0] };
        } catch (error) {
            await session.abortTransaction();
            session.endSession();
    ...
    async updateOne(id: string, updateDto: UpdateCourseDto) {
        try {
            ...
            if (!updateResult) return { statusCode: 404, message: 'Course not found' };
    ...
    async softDelete(id: string) {
        try {
            const deleteResult = await this.courseModel.findOneAndUpdate(
                {
                    _id: new mongoose.Types.ObjectId(id),
                    isDeleted: false
                },
            ...
            if (!deleteResult) return { statusCode: 404, message: 'Course not found' };
